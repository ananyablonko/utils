...
def should_run_agent(keys: list[str], callback_context: CallbackContext, inverse: bool = False) -> Optional[types.Content]:
    f"""
    USAGE
    -----
    ...
    """
    for key in keys:
        if (not inverse) ^ (key in callback_context.state):
